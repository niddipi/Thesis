		...
		while not rospy.is_shutdown():
			if (self.status == DroneStatus.Landed):
				...
				while rospy.get_time() <= start_time+3.0:
					print(" OK ")
				start_time = rospy.get_time()
			elif rospy.get_time() <= start_time+1.0:
				self.move_UP()

			elif rospy.get_time() <= start_time+2.0:
				self.move_Down()
				if start_time+2.0 == rospy.get_time():
					start_time = rospy.get_time()
			
			if rospy.get_time() > Begin+9.5:
...
if __name__=='__main__':
	import sys
	rospy.init_node('ardrone1', anonymous=True)
